...
| Dashboard UI                 | ✅ Done        | Sidebar nav (fixed links), overview page with live stats/recent applications, jobs listing with status filter tabs + count badges, candidates table from real applications. Premium metric cards with animated icons. |
| Dashboard Design System      | ✅ Done        | Unified metric card design across demo and real dashboards. Size-6 animated icons, clean spacing, supporting descriptions, consistent typography (text-2xl values, text-sm titles).                                   |
...
| Product Preview Demo         | ✅ Done        | Interactive hiring dashboard preview with live filtering, animated metric cards, trend charts (AreaChart), pipeline breakdown (BarChart), and recent applications table.                                              |
| Admin Dashboard (Module 5)   | ✅ Done        | Platform-only `/admin` with email allowlist auth, overview, users/orgs, queue, candidate debug, and system health. Metric cards match main dashboard design.                                                          |
